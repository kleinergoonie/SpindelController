...
$Form.Text = "MT6835 Stream GUI (fixed3)"
...
function Append-Log([string]$text) {
    ...
    try {
        [System.Threading.Tasks.Task]::Run({ [System.IO.File]::AppendAllText($script:logFile, $t, [System.Text.Encoding]::UTF8) }) | Out-Null
    } catch { }
}
...
function Safe-Write([string]$s) {
    if (-not $script:serialPort -or -not $script:serialPort.IsOpen) { throw "Not open" }
    try {
        # write without newline to avoid firmware newline parsing issues
        $script:serialPort.Write($s)
    } catch {
        Append-Log ("Write failed: " + ($_.Exception.Message))
        throw $_
    }
}
...
function Disconnect-Port {
    try {
        ...
        if ($script:serialPort -and $script:serialPort.IsOpen) {
            if ($script:streaming) {
                try { Safe-Write('s') } catch { }
                $script:streaming = $false
                Start-Sleep -Milliseconds 50
            }
            try { $script:serialPort.Close() } catch { }
...
$btnConnect.Add_Click({
    if (-not $script:serialPort) {
        ...
        try {
            ...
            # do NOT set DTR/RTS to avoid auto-reset on some adapters/boards
...
try { [System.Threading.Tasks.Task]::Run({ [System.IO.File]::AppendAllText($script:logFile, ('==== ' + (Get-Date).ToString('yyyy-MM-dd HH:mm:ss') + ' GUI Start ====') + "`r`n", [System.Text.Encoding]::UTF8) }) | Out-Null } catch { }
