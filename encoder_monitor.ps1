...
$form.Text = "Encoder Monitor (Auto-detect)"
...
$toolTip.SetToolTip($lblMdTitle, "MAG / UEBERD: Magnet/Ãœberdrehzahl (1 = Ereignis)")
$toolTip.SetToolTip($lblMlTitle, "SCHW: Magnet zu schwach (1 = zu schwach)")
$toolTip.SetToolTip($lblMhTitle, "STARK / UV: Magnet zu stark oder Unterspannung (1 = Fehler)")
...
Update-PortList
# Auto-connect when a Port parameter is supplied (e.g. -Port COM5).
if ($Port) {
    if ($cmbPort.Items.Contains($Port)) {
        ...
        try {
            ...
            Write-LogLine "Auto-connect failed: $($_.Exception.Message)"
        }
    }
    else {
        Write-LogLine "Port $Port not found"
    }
}
